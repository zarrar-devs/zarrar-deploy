...
/* =============================================================
   ...
   Layout differs from the other persona pages in three places:
   the hero (single masked-line reveal + a search-terms strip),
   the portal-vs-owned comparison, and a 4-card services grid
   with a local-SEO card.
   ...
   - Nav "Contact", each service card's CTA, and each plan's
     ...
     smooth-scrolled to the closing section via onAnchorClick below.
   - They're now plain <button> elements with no href at all — they
     open the contact modal directly instead of scrolling, so
     onAnchorClick (which only matches a[href^="#"]) no longer
     touches them.
   - The closing section's own "Email us" button is untouched — it's
...
// in your project (same component used on the other persona pages).
...
const ICONS = { web: WebIcon, pin: MapPinIcon, outreach: OutreachIcon, social: SocialIcon };
    ...
    const ctx = gsap.context((self) => {
      ...
      mm.add(
        ...
        (mq) => {
          ...
          /* ---- load sequence: single masked-line hero on desktop, simple rise on phones ---- */
          gsap
            ...
            .add(
              heavy ? linesIn(q(".hero-heading")[0]) : riseIn(q(".hero-heading")[0], {}, 28),
              0.12
            )
            .add(textIn(q(".hero-sub")[0]), 0.42)
            .from(q(".area-chip"), { y: 14, opacity: 0, duration: 0.5, stagger: 0.05 }, 0.65)
          ...
          /* ---- compare columns: slide in from opposite sides on desktop, rise on phones ---- */
          const compareCols = q(".compare-col");
          compareCols.forEach((col, i) => {
            gsap.from(col, {
              ...(heavy ? { xPercent: i ? 6 : -6 } : { y: 30 }),
              opacity: 0,
              duration: 0.9,
              ease: "power3.out",
              delay: heavy ? i * 0.12 : 0,
              scrollTrigger: {
                trigger: heavy ? q(".compare-grid")[0] : col,
                start: "top 82%",
                once: true,
              },
            });
            gsap.from(col.querySelectorAll("li"), {
              ...
              ease: "power3.out",
              stagger: 0.06,
              scrollTrigger: { trigger: col, start: "top 76%", once: true },
            });
          });
          ...
          cards.forEach((card) => {
            if (heavy) flipIn(card.querySelector("h3"), { trigger: card, start: "top 84%" });
      ...
      <main id="main" tabIndex={-1}>
          ...
          <div className="area-chips">
            ...
            <ul className="area-chips-list" aria-labelledby="area-chips-label">
              {SEARCH_TERMS.map((term) => (
                <li className="area-chip" key={term}>
                  {term}
                </li>
              ))}
            </ul>
        ...
        <section className="compare" id="compare" aria-labelledby="compare-title">
          ...
          <div className="compare-grid">
            <div className="compare-col">
              <h3 className="compare-tag compare-tag-old">{COMPARE.old.tag}</h3>
              <ul>
                {COMPARE.old.points.map((point) => (
                  <li key={point}>{point}</li>
                ))}
              </ul>
            </div>
            <div className="compare-col compare-col-highlight">
              <h3 className="compare-tag compare-tag-new">{COMPARE.owned.tag}</h3>
              <ul>
                {COMPARE.owned.points.map((point) => (
                  <li key={point}>{point}</li>
                ))}
              </ul>
            </div>
        ...
        <section className="case-study" id="process" aria-labelledby="case-title">
          ...
          <p className="case-note">
            Timing is a guide. The exact pace depends on your market, price
        ...
        <section className="faq" id="faq" aria-labelledby="faq-title">
          <div className="section-head">
            <h2 id="faq-title">Questions real estate agents ask us</h2>
